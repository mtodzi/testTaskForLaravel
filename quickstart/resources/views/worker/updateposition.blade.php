        ...
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h4>Обновление должности шаг - 2</h4>            
                </div>
                <div class="panel-body">
                <form action="{{ url('/updateworkers/'.$worker->id) }}" method="POST">
                    {{ csrf_field() }}
                ...
                {{empty($worker->id_position)?"Должность не задана":"Должность - ".$worker->position->name_position}}    
                <form  id="my_form" action="{{ url('/updateworkers/'.$worker->id) }}" method="POST">
                    {{ csrf_field() }}
                    <input id = "worker" type="hidden" value = "{{$worker->id}}" name="worker" class="form-control">
